...
"""
original file: verl/trainer/ppo/ray_trainer.py
"""
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        device_name=None,
        skip_dataloader_init: bool = False,
    ):
        """
        ...
        Args:
            ...
            skip_dataloader_init (bool, optional): If True, skip dataloader initialization.
                Useful when using custom dataloaders (e.g., HTTP client). Defaults to False.
        """
        ...
        self._create_dataloader(
            ...
            collate_fn,
            train_sampler,
            skip_init=skip_dataloader_init,
        )

    def post_init(self, total_training_steps):
    ...
    def _create_dataloader(
        ...
        Args:
            ...
            skip_init: If True, skip dataloader creation and set default attributes
        """
        # Skip dataloader creation if requested (e.g., for HTTP server with custom client dataloaders)
        if skip_init:
            self.train_dataset = None
            self.val_dataset = None
            self.train_dataloader = None
            self.val_dataloader = None
            # self.total_training_steps = self.config.trainer.total_epochs
            self.total_training_steps = None
            print("Skipped dataloader initialization (using custom dataloader)")
            return
    ...
    def init_workers(self):
        ...
        if self.config.actor_rollout_ref.rollout.mode == "async":
            from opentinker.backend_patch.verl.experimental.agent_loop.agent_loop import AgentLoopManager
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                print("tensor keys:", list(batch.batch.keys()))
                print("non-tensor keys:", batch.non_tensor_batch.keys())
                print("meta_info:", batch.meta_info)
                # print("extra_fields:", batch.extra_fields)
                # add uid to batch
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("old_log_prob", timing_raw, color="blue"):
                        ...
                        print("=" * 80)
                        print(
                            f"[STANDARD] Batch state before compute_log_prob (step {self.global_steps})"
                        )
                        ...
                        # ===== DEBUG LOGGING START =====
                        print(
                            f"[STANDARD] After compute_log_prob (step {self.global_steps})"
                        )
                        print(f"  entropys shape: {entropys.shape}")
                        print(f"  entropys mean: {entropys.mean().item()}")
                        print(f"  entropys std: {entropys.std().item()}")
                        print(f"  entropy_agg (actor/entropy): {entropy_agg.item()}")
                        print(
